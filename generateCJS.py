...
def generate():
    jobCount = [ 20, 40, 60, 100, 200 ]
    capacities = [ 2, 4, 6 ]

    for i in range(15):
        g = Generator();

        g.setNmbJobs(jobCount[int(i / 3)])
        g.setNmbMachines(g.nmbJobs / 20, g.nmbJobs / 10)
        g.setNmbFamilies(g.nmbJobs / 10, g.nmbJobs / 5)

        g.setBatchCapacity(capacities[int(i % 3)])

        g.generate("random" + ("%02d" % (i+1)) + ".cjs.input")
